...
class VolumeOps(ops_task_base.OpsTaskBase):
    ...
    def create_local_disk_batch(self, **kwargs):
        ...
        body = {'clusterUrn': kwargs.get('cluster_urn'), 'numberOfVolumes': kwargs.get('local_disk_count'),
                'volumeSize': kwargs.get('local_disk_size'), 'type': kwargs.get('local_disk_type'),
                'safeFormat': safe_format, 'volumeUrns': kwargs.get('volume_urns'), 'vmUrn': kwargs.get('fc_vm_urn'),
    ...
    def need_del_backup_snapshots(self, snapshot_info_list, volume_urn):
        ...
        snapshots_with_volume = filter(lambda x: _is_vol_in_snap(x,volume_urn), snapshot_info_list)
        if snapshots_with_volume is None or len(snapshots_with_volume)==0:
            LOG.info("can't find volume %s in snapshot %s"%(volume_urn, snapshot_info_list))
            return []
        ...
        for snapshot in snapshots_with_volume:
            ...
            if (type!='backup' and type!='CBTbackup') or status !='ready':
                msg = _('snapshot is %s and status is %s, do not del any snapshot')%(type, status)
                LOG.info(msg)
                raise fc_exc.InvalidSnapshotInfo(msg)
    ...
    def pre_detach_volume(self, snapshot_lock, instance_url, volume_url):
        ...
        def _do_pre_detach_volume(instance_url, volume_url):
            ...
            need_del_snap = self.need_del_backup_snapshots(snap_infos, volume_url)
