...
class ClusterOps(ops_task_base.OpsTaskBase):
    ...
    def list_all_clusters(self):
        ...
        :return:
        """
        LOG.info('list_all_clusters self.site.cluster_uri:%s .' % self.site.cluster_uri)
    ...
    def get_resource_group(self, cluster_urn, instance_group):
        ...
        resource_group_uri = utils.generate_uri_from_urn(cluster_urn) + '/resourcegroups'
        condition = {'type': 0, 'useType': 1, 'name': instance_group['uuid'], 'limit': 100, 'offset': 0}
        resource_group_uri = utils.build_uri_with_params(resource_group_uri, condition)
    ...
    def get_resource_group_list(self, cluster_urn):
        ...
        while True:
            ...
            if total > 0:
                ...
                if offset >= total or len(resourcegroups_all) >= total or len(resourcegroups) < limit:
                    break
            else:
                break
    ...
    def create_resource_group(self, cluster_urn, instance_group):
        ...
        resource_group_uri = utils.generate_uri_from_urn(cluster_urn) + '/resourcegroups'
        body = {'type': 0, 'useType': 1, 'name': instance_group['uuid'], 'policies': instance_group.get('policies')}
    ...
    def get_cluster_stats_by_name(self, cluster_name):
        ...
        if find_cluster:
            ...
            if allocated_cpu_detail is not None:
                cpu_info["stopVmAllocatedVcpus"] = allocated_cpu_detail.get('allocatedVcpus').get("Stopped")
            ...
            if allocated_mem_detail is not None:
                mem_info["stopVmAllocatedMem"] = allocated_mem_detail.get('allocatedSizeMB').get("Stopped")
    ...
    def get_cluster_resource(self, cluster_name):
        ...
        res['vcpus_used'] = self._calculate_vcpu_mem_used(cluster_stats["cpu"]['stopVmAllocatedVcpus'],cluster_stats["cpu"]["allocatedVcpus"])
        res['memory_mb_used'] = self._calculate_vcpu_mem_used(cluster_stats["mem"]['stopVmAllocatedMem'],cluster_stats['mem']['used'])
    ...
    def _calculate_vcpu_mem_used(self, stopped_vm_allocated, all_vm_allocated):
        ...
        if constant.CONF.fusioncompute.resource_reduced_rate is not None:
            resource_reduced_rate = constant.CONF.fusioncompute.resource_reduced_rate
        return all_vm_allocated - stopped_vm_allocated \
               + math.ceil(stopped_vm_allocated * float(resource_reduced_rate) / 100)
