...
class FCInstanceOps(ops_base.OpsBase):
    ...
    def get_all_vms_info(self,**kwargs):
        ...
        while len(states) < total:
            ...
            for instance in instances.get('vms'):
                if instance.get('params') is not None and instance.get('params').get("externalUuid") is not None:
                    states[instance["params"]['externalUuid']] = constant.VM_POWER_STATE_MAPPING.get(instance['status'], power_state.NOSTATE)
                else:
                    states[instance['uuid']] = constant.VM_POWER_STATE_MAPPING.get(instance['status'], power_state.NOSTATE)
    ...
    def get_vm_by_uuid(self, instance):
        ...
        return self._get_fc_vm_by_uuid_and_external_uuid(instance['uuid'], externalUuid=instance['uuid'])
    ...
    def _get_fc_vm_by_uuid_and_external_uuid(self, vm_info, limit=1, offset=0, detail=2, **kwargs):
        """
        get fv vm info by conditions
        :param vm_info:
        :param limit:
        ...
        instances = self._query_vm(limit=limit, offset=offset, detail=detail, **kwargs)
        if not instances or not instances['vms']:
            instances_by_uuids = self._query_vm(limit=limit, offset=offset, detail=detail,
                                  uuid=vm_info)
