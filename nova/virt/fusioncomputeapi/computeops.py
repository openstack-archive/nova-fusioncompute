...
class ComputeOps(ops_task_base.OpsTaskBase):
    ...
    def _modify_boot_option_if_needed(self, instance, fc_vm):
        ...
        if new_boot_option and old_boot_option and \
           new_boot_option != old_boot_option:
            LOG.info(_("trying to modify boot option from %s "
                       "to %s") % (old_boot_option, new_boot_option))
            body = {
                'properties': {
        ...
        if new_vnc_keymap_setting and old_vnc_keymap_setting and \
           new_vnc_keymap_setting != old_vnc_keymap_setting:
            LOG.info(_("trying to modify vnc_keymap setting from %s "
                       "to %s") % (old_vnc_keymap_setting, new_vnc_keymap_setting))
            body = {
                'properties': {
    ...
    def get_vnc_console(self, instance, get_opt):
        ...
        if get_opt is False:
            LOG.debug(_("return vnc info is host: %s, "
                        "port:%s,"
        ...
        LOG.debug(_("return get vnc info is host: %s, "
                    "port:%s,"
                    " internal_access_path: %s"),
                  host_ip, host_port, 'None')
