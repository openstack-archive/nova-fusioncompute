...
class ComputeOps(ops_task_base.OpsTaskBase):
        ...
        try:
            ...
            if local_disk_property:
                instance.system_metadata.update(
                    {'local_disk_property':
                         jsonutils.dumps(local_disk_property)})
            instance.save()
        except Exception as exc:
            utils.log_exception(exc)
        ...
        if resource_group:
            ...
            try:
                ...
                if resource_group:
                    ...
                    LOG.error(
                        _("Create resource group failed for %s ."
                          "Exception = %s"),
                        instance_group['uuid'],
                        ex)
                    raise ex
        ...
        if disk_urns:
            for disk_urn in disk_urns:
                ...
                try:
                    ...
                    LOG.error(
                        _("delete volume %s of vm failed,"
                          " exception is %s."),
                        disk_urn,
                        ex)
    ...
    def _modify_boot_option_if_needed(self, instance, fc_vm):
        ...
        if new_boot_option and old_boot_option and \
           new_boot_option != old_boot_option:
            LOG.info(_("trying to modify boot option from"
                       " %s to %s") %
    ...
    def _modify_vnc_keymap_setting_if_needed(self, instance, fc_vm):
        ...
        if new_vnc_keymap_setting and old_vnc_keymap_setting and \
           new_vnc_keymap_setting != old_vnc_keymap_setting:
            LOG.info(_("trying to modify vnc_keymap setting from %s "
                       "to %s") %
    ...
    def pause_vm(self, instance):
        ...
        if self.get_local_disk_property(instance):
            reason = _("vm %s can not be resized due to it has local disk.")\
                     % instance['uuid']
    ...
    def _delete_vm_with_fc_vm(
        ...
        def _delete_vm():
            ...
            def _delete_vm_with_timelimited():
                ...
                LOG.info(_('Deleting VM on FC, instance: %s '
                           'reserve_disks %s'),
    ...
    def live_migration(self, context, instance, hostname, post_method,
        ...
        try:
            LOG.info(_("trying to live migrate vm %s "
                       "to host %s."),
    ...
    def modify_instance_vnic(self, fc_vm, old_flavor, new_flavor):
        if old_flavor.get('instance_bandwidth') != new_flavor.get('instance_bandwidth') \
                or old_flavor.get('instance_max_vnic') \
                        != new_flavor.get('instance_max_vnic'):
    ...
    def modify_vm_gpu(self, fc_vm, src_flavor, dest_flavor):
        ...
        if src_gpu_mode is not None and dest_gpu_mode is not None and \
                        src_gpu_mode != dest_gpu_mode:
    ...
    def attach_volume(self, connection_info, instance, mountpoint):
        ...
        if self._check_if_vol_in_instance(fc_vm, vol_urn) is True:
            LOG.info(_("vm %s already has "
                       "vol %s, consider it success"),
    ...
    def detach_volume(self, connection_info, instance, is_snapshot_del=True):
        ...
        if self._check_if_vol_in_instance(fc_vm, vol_urn) is False:
            LOG.info(_("vol %s is not in "
                       "vm %s, consider it success"),
