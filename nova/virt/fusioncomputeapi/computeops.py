...
class ComputeOps(ops_task_base.OpsTaskBase):
    ...
    def _init_os_config(self):
        ...
        constant.HUAWEI_VIRTUAL_IO_OS_VERSION_INT(config_file=constant.VIRTUAL_IO_OS_CONFIG_FILE)
        ...
        if constant.CONF.fusioncompute.enable_virtualio:
            for os_type in constant.HUAWEI_VIRTUAL_IO_OS_VERSION_INT.keys():
                for os_version in constant.HUAWEI_VIRTUAL_IO_OS_VERSION_INT[os_type].values():
    ...
    def create_vm(self, context, instance, network_info, block_device_info,
        ...
        for idx, network_item in enumerate(network_info):
            LOG.info(_('create_vm prepare network [%s]' % network_item))
            ...
            pg_urn = self._network_ops.ensure_network(network_item['network'], checksum_enable, extra_specs)
        ...
        resource_group_urn = self.ensure_instance_group(instance, cluster_urn=location)
        ...
        try:
            ...
            vm_create(context, self._volume_ops, location, vifs,
                ...
                admin_password, extra_specs, customization, resource_group_urn,self)
            vm_create.create_and_boot_vm()
        except Exception as exc:
            utils.log_exception(exc)
            msg = _("create and boot vm %s failed.") % instance['uuid']
            self.delete_vm(context, instance, block_device_info, is_need_check_safe_format=False)
        ...
        try:
            ...
            if local_disk_property:
                instance.system_metadata.update({'local_disk_property': jsonutils.dumps(local_disk_property)})
            instance.save()
        except Exception as exc:
            utils.log_exception(exc)
    ...
    def ensure_resource_group(self, cluster_urn, instance_group):
        ...
        resource_group = self._cluster_ops.get_resource_group(cluster_urn, instance_group)
        if resource_group:
            return resource_group.get('urn')
        else:
            try:
                return self._cluster_ops.create_resource_group(cluster_urn, instance_group)
                ...
                if resource_group:
                    ...
                    LOG.error(_("Create resource group failed for %s .Exception = %s"), instance_group['uuid'], ex)
                    raise ex

    def ensure_instance_group(self, instance, cluster_urn=None):
        instance_group = self.get_instance_group_by_instance_uuid(instance['uuid'])
        if instance_group:
            if cluster_urn is None:
                cluster_urn = self._cluster_ops.get_cluster_urn_by_nodename(instance['node'])
            return self.ensure_resource_group(cluster_urn, instance_group)

    def get_instance_group_by_instance_uuid(self, instance_uuid, read_deleted='no'):
        """
    ...
    def cleanup_deleted_resource_group_by_instance(self, instance, cluster_urn=None):
        instance_group = self.get_instance_group_by_instance_uuid(instance['uuid'], read_deleted='only')
        if instance_group:
            if cluster_urn is None:
                cluster_urn = self._cluster_ops.get_cluster_urn_by_nodename(instance['node'])
            resource_group = self._cluster_ops.get_resource_group(cluster_urn, instance_group)
    ...
    def stop_vm_with_timelimited(self, instance, force=False):
        ...
        try:
            ...
            LOG.info(_("first %s stop vm %s success"), body['mode'], instance['uuid'])
            return
        except Exception as ex:
            LOG.error(_("stop vm %s failed, reason is %s."), instance['uuid'], ex)
    ...
    def stop_vm(self, instance, force=False):
        ...
        if fc_vm.status == constant.VM_STATUS.STOPPED:
            ...
            LOG.info(_("safe stop this vm."))
            try:
                self.post(fc_vm.get_vm_action_uri('stop'), data=body, excp=exception.InstancePowerOffFailure)
                LOG.info(_("first %s stop vm %s success"), body['mode'], instance['uuid'])
                return
            except exception.InstancePowerOffFailure:
                LOG.error(_("first stop vm %s failed, will force stop."), instance['uuid'])
            # if first stop failed, force stop
            body = {'mode': 'force'}
        try:
            self.post(fc_vm.get_vm_action_uri('stop'), data=body,
                      excp=exception.InstanceFaultRollback)
            LOG.info(_("%s stop vm %s success"), body['mode'], instance['uuid'])
            return
        except Exception as ex:
            LOG.error(_("stop vm %s failed, reason is %s."), instance['uuid'], ex)
            raise exception.InstanceFaultRollback
    ...
    def get_local_disk_property(self, instance):
        if instance.system_metadata.get('local_disk_property'):
            return jsonutils.loads(instance.system_metadata.get('local_disk_property'))
        result = {}
        ...
        if disk_urns:
            for disk_urn in disk_urns:
                ...
                try:
                    ...
                    LOG.error(_("delete volume %s of vm failed, exception is %s."), disk_urn, ex)
    ...
    def _modify_vnc_keymap_setting_if_needed(self, instance, fc_vm):
        ...
        if 'vmConfig' in fc_vm:
            vm_property = fc_vm['vmConfig'].get('properties')
            old_vnc_keymap_setting = vm_property.get('vmVncKeymapSetting') if vm_property \
                              else None

        if new_vnc_keymap_setting and old_vnc_keymap_setting and \
           new_vnc_keymap_setting != old_vnc_keymap_setting:
            LOG.info(_("trying to modify vnc_keymap setting from %s to %s") %
                     (old_vnc_keymap_setting, new_vnc_keymap_setting))
            body = {
                'properties': {
            ...
            try:
                ...
                LOG.error(_("modify vnc_keymap setting has exception: %s") % msg)
    ...
    def start_vm(self, instance, block_device_info=None):
        ...
        if fc_vm.status in [constant.VM_STATUS.STOPPED,
            ...
            try:
                ...
                LOG.error(ex)
                reason = _("FusionCompute start vm %s failed") % instance['uuid']
    ...
    def create_and_attach_local_disk_before_start(self, instance, block_device_info):
        ...
        if local_disk_property:
            ...
            cinder_volume_urns = self._get_vol_urns_from_block_device_info(block_device_info)

            volume_urns = self._volume_ops.create_local_disk_batch(cluster_urn=cluster_urn,
                                                                   ...
                                                                   fc_vm_urn=fc_vm.urn,
                                                                   local_disk_safe_format=local_disk_property.get(
                                                                       'safe_format'))
            unbind_volume_urns = self.attach_local_disk_batch(volume_urns, fc_vm)
            if unbind_volume_urns is not None and len(unbind_volume_urns) > 0:
                self.delete_local_disk(unbind_volume_urns)
                reason = _("vm %s need to attach local disk before power on but failed.") % fc_vm.uuid
                raise exception.InstancePowerOnFailure(reason=reason)
    ...
    def _get_vol_urns_from_block_device_info(self, block_device_info):
        ...
        if block_device_info and block_device_info.get('block_device_mapping'):
            LOG.info(_('create local disk block device info is %s.'), str(block_device_info))
            for vol in block_device_info.get('block_device_mapping'):
                vol_urn = self._get_vol_urn_from_connection(vol.get('connection_info'))
    ...
    def get_sequence_num_local_disk(self, last_sequence_num):
        ...
        if constant.CONF.fusioncompute.reserve_disk_symbol is None \
                or str(constant.CONF.fusioncompute.reserve_disk_symbol).upper() == 'TRUE':
    ...
    def reboot_vm(self, instance, reboot_type, block_device_info):
        ...
        LOG.debug(_("reboot_vm %s, reboot_type %s, fc_vm.status %s."),
                  instance['uuid'], reboot_type, fc_vm.status)
    ...
    def pause_vm(self, instance):
        ...
        if self.get_local_disk_property(instance):
            reason = _("vm %s can not be resized due to it has local disk.") % instance['uuid']
    ...
    def suspend_vm(self, instance):
        ...
        if fc_vm.status == constant.VM_STATUS.RUNNING:
            try:
                ...
                LOG.error(_("Fc_vm is running ,but suspending vm is error.The reason is %s.") % ex)
                raise exception.InstanceFaultRollback
    ...
    def _delete_vm_with_fc_vm(self, instance, destroy_disks=True, is_need_check_safe_format=True):
        """
        ...
        def _delete_vm():
            ...
            @utils.timelimited(constant.CONF.fusioncompute.fc_request_timeout_delete_vm_timelimited)
            def _delete_vm_with_timelimited():
                ...
                if local_disk_property:
                    local_disk_count = local_disk_property.get('count')
                local_disk_sequence_nums = self.get_sequence_nums_local_disk(local_disk_count)
                try:
                    for disk in fc_vm['vmConfig']['disks']:
                        if disk['sequenceNum'] > 1 and disk['sequenceNum'] not in local_disk_sequence_nums:
                            LOG.info(_('Detach leaked volume: %s'), disk['volumeUrn'])
                ...
                if destroy_disks and is_need_check_safe_format is True:
                    reserve_disks['isFormat'] = self._is_disk_safe_format(instance)
                LOG.info(_('Deleting VM on FC, instance: %s reserve_disks %s'),
                    fc_vm.uuid, jsonutils.dumps(reserve_disks))
    ...
    @utils.timelimited(constant.CONF.fusioncompute.fc_request_timeout_delete_vm)
    def delete_vm(self, context, instance, block_device_info=None,
        ...
        if instance and (instance.get('task_state') == 'resize_reverting'
            ...
            try:
                ...
                LOG.warn(_('safe stop vm failed, trigger force stop vm. %s'), e)
                try:
        ...
        self._delete_vm_with_fc_vm(instance, destroy_disks, is_need_check_safe_format)
    ...
    def _find_destination_node(self, context, instance, host):
        ...
        try:
            ...
            nodename = self.scheduler_client.select_destinations(context,
                _build_request_spec(), filter_properties)[0]['nodename']
        except Exception as e:
    ...
    def live_migration(self, context, instance, hostname, post_method,
        ...
        try:
            LOG.info(_("trying to live migrate vm %s to host %s."),
            ...
            cluster_urn = self._cluster_ops.get_cluster_urn_for_migrate(nodename)
            if not cluster_urn:
                raise fc_exc.ClusterNotFound(cluster_name=nodename)

            if self.get_local_disk_property(instance):
                LOG.error(_("vm %s can not be live migrated due to it has local disk."), instance['uuid'])
                raise exception.MigrationError

            self.cleanup_deleted_resource_group_by_instance(instance, cluster_urn=cluster_urn)

            resource_group_urn = self.ensure_instance_group(instance, cluster_urn=cluster_urn)
            ...
            post_method(context, instance, hostname, block_migration, migrate_data)
            LOG.info(_("Live Migration success: %s"), instance['uuid'])
        except Exception as e:
            with excutils.save_and_reraise_exception():
                LOG.error(_("Live Migration failure: %s"), e, instance=instance)
    ...
    def migrate_disk_and_power_off(self, instance, dest, flavor, block_device_info):
        """
        ...
        if self.get_local_disk_property(instance):
            LOG.error(_("vm %s can not be resized due to it has local disk."), instance['uuid'])
            raise exception.InstanceFaultRollback
    ...
    def finish_migration(self, instance, power_on=True, block_device_info=None):
        """
        ...
        self.cleanup_deleted_resource_group_by_instance(instance, cluster_urn=location)
    ...
    def finish_revert_migration(self, instance, power_on=True, block_device_info=None):
        """
    ...
    def modify_instance_vnic(self, fc_vm, old_flavor, new_flavor):
        if old_flavor.get('instance_bandwidth') != new_flavor.get('instance_bandwidth') \
                or old_flavor.get('instance_max_vnic') != new_flavor.get('instance_max_vnic'):
            uri = fc_vm.uri + '/simplespec'
            body = {'vmParams': {'bandwidth': new_flavor.get('instance_bandwidth'),
    ...
    def modify_vm_gpu(self, fc_vm, src_flavor, dest_flavor):
        ...
        src_gpu_num = src_flavor.get('gpu_num') if src_flavor.get('gpu_num') else 0
        src_gpu_mode = src_flavor.get('gpu_mode')
        dest_gpu_num = dest_flavor.get('gpu_num') if dest_flavor.get('gpu_num') else 0
        ...
        if src_gpu_mode is not None and dest_gpu_mode is not None and src_gpu_mode != dest_gpu_mode:
        ...
        if src_gpu_num > dest_gpu_num:
            ...
            for i in range(dest_gpu_num - src_gpu_num):
                self.post(attach_gpu_uri, data={'gpuUrn': 'auto', 'mode': dest_gpu_mode},
                         excp=fc_exc.InstanceModifyFailure)
        ...
        if flavor:
            if flavor.get('vcpus'):
                ...
                if numa_nodes is not None:
                    _core_per_socket = int(flavor.get('vcpus'))/int(numa_nodes)
            ...
            if flavor.get('memory_mb'):
                ...
                if flavor.get('gpu_num') > 0:
                    data['memory'].update({'reservation': flavor.get('memory_mb')})

            cpu_qos = utils.fc_qos_convert(flavor, constant.CPU_QOS_FC_KEY,
                                           constant.CPU_QOS_FC_KEY, flavor.get('vcpus'))
    ...
    def _gen_old_flavor_for_fc(self, instance):
        ...
        if gpus:
            flavor_dict.update({'gpu_num': len(gpus), 'gpu_mode': gpus[0].get('mode')})
        ...
        if properties:
            if properties.get("secureVmType"):
                flavor_dict.update({'secureVmType': properties.get("secureVmType")})
    ...
    def _gen_new_flavor_for_fc(self, flavor):
        ...
        if extra_specs:
            ...
            if socketNum:
                flavor_dict = utils.dict_add(flavor_dict, {'socketNum': socketNum})
            ...
            if enable_gpu and str(enable_gpu).upper() == 'TRUE':
                if gpu_specs:
                    ...
                    if gpu_specs and len(gpu_specs) == 3:
                        ...
                        gpu_num = gpu_specs[2]
            if gpu_num:
                flavor_dict.update({'gpu_num': int(gpu_num), 'gpu_mode': gpu_mode})
            ...
            if instance_vnic_type and instance_vnic_type.lower() == 'enhanced':
                instance_bandwidth = extra_specs.get('instance_vnic:instance_bandwidth')
                instance_max_vnic = extra_specs.get('instance_vnic:max_count')

            if instance_bandwidth:
                flavor_dict.update({'instance_bandwidth': int(instance_bandwidth)})
            if instance_max_vnic:
                flavor_dict.update({'instance_max_vnic': int(instance_max_vnic)})
    ...
    def attach_interface(self, instance, vif, extra_specs):
        ...
        """
        LOG.info(_("trying to attach interface, vm name: %s,"
        ...
        pg_urn = self._network_ops.ensure_network(vif['network'], checksum_enable, extra_specs)
        vsp_body = {
            'name': vif['id'],
            'portId': vif['id'],
            'portGroupUrn': pg_urn,
            'mac': vif['address'],
            'virtIo': 1 if str(fc_vm.osOptions.get('osVersion')) in constant.VIRTUAL_IO_OS_LIST else 0
        }
    ...
    def get_info(self, instance):
        ...
        if fc_vm.status == constant.VM_STATUS.STOPPING:
            ...
            if vm_params is not None:
                ...
                if notify_ret is not None and notify_ret == constant.NOTIFY_NEUTRON.FALSE:
    ...
    def get_sequence_num(self, vol_urn, mountpoint):
        ...
        if fc_volume:
            if fc_volume.get('pvscsiSupport') == 1 or fc_volume.get('storageType') == 'LUN':
                return constant.MOUNT_DEVICE_SEQNUM_MAP.get(mountpoint)
            else:
