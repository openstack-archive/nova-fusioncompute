...
class ComputeOps(ops_task_base.OpsTaskBase):
        ...
        try:
            ...
            if local_disk_property:
                instance.system_metadata.update(
                    {'local_disk_property': jsonutils.dumps(local_disk_property)})
            instance.save()
        except Exception as exc:
            utils.log_exception(exc)
        ...
        if resource_group:
            ...
            try:
                ...
                if resource_group:
                    ...
                    LOG.error(
                        _("Create resource group failed for %s .Exception = %s"),
                        instance_group['uuid'],
                        ex)
                    raise ex
    ...
    def ensure_instance_group(self, instance, cluster_urn=None):
        ...
        if instance_group:
            if cluster_urn is None:
                cluster_urn = self._cluster_ops.get_cluster_urn_by_nodename(instance[
                                                                            'node'])
    ...
    def stop_vm_with_timelimited(self, instance, force=False):
        ...
        try:
            ...
            LOG.info(
                _("first %s stop vm %s success"),
                body['mode'],
                instance['uuid'])
            return
        except Exception as ex:
            LOG.error(
                _("stop vm %s failed, reason is %s."),
                instance['uuid'],
                ex)
        ...
        if disk_urns:
            for disk_urn in disk_urns:
                ...
                try:
                    ...
                    LOG.error(
                        _("delete volume %s of vm failed, exception is %s."),
                        disk_urn,
                        ex)
    ...
    def create_and_attach_local_disk_before_start(
        ...
        if local_disk_property:
            ...
            if unbind_volume_urns is not None and len(unbind_volume_urns) > 0:
                ...
                reason = _(
                    "vm %s need to attach local disk before power on but failed.") % fc_vm.uuid
                raise exception.InstancePowerOnFailure(reason=reason)
    ...
    def get_sequence_num_local_disk(self, last_sequence_num):
        ...
        if constant.CONF.fusioncompute.reserve_disk_symbol is None or str(
                constant.CONF.fusioncompute.reserve_disk_symbol).upper() == 'TRUE':
    ...
    def pause_vm(self, instance):
        ...
        if self.get_local_disk_property(instance):
            reason = _("vm %s can not be resized due to it has local disk.") % instance[
                'uuid']
    ...
    def suspend_vm(self, instance):
        ...
        if fc_vm.status == constant.VM_STATUS.RUNNING:
            try:
                ...
                LOG.error(
                    _("Fc_vm is running ,but suspending vm is error.The reason is %s.") %
                    ex)
                raise exception.InstanceFaultRollback
    ...
    def _delete_vm_with_fc_vm(
        ...
        def _delete_vm():
            ...
            @utils.timelimited(
                constant.CONF.fusioncompute.fc_request_timeout_delete_vm_timelimited)
    ...
    def _find_destination_node(self, context, instance, host):
        ...
        try:
            ...
            nodename = self.scheduler_client.select_destinations(
                context, _build_request_spec(), filter_properties)[0]['nodename']
        except Exception as e:
    ...
    def live_migration(self, context, instance, hostname, post_method,
        ...
        try:
            ...
            if self.get_local_disk_property(instance):
                LOG.error(
                    _("vm %s can not be live migrated due to it has local disk."),
                    instance['uuid'])
    ...
    def attach_interface(self, instance, vif, extra_specs):
        ...
        vsp_body = {
            ...
            'mac': vif['address'],
            'virtIo': 1 if str(
                fc_vm.osOptions.get('osVersion')) in constant.VIRTUAL_IO_OS_LIST else 0}
