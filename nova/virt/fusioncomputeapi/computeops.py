...
class ComputeOps(ops_task_base.OpsTaskBase):
    ...
    def stop_vm_with_timelimited(self, instance, force=False):
        ...
        try:
            ...
            LOG.error("stop vm %s failed", instance['uuid'])
    ...
    def migrate_disk_and_power_off(
        ...
        # check vgpu params
        if old_flavor.get('gpu_num') != new_flavor.get(
                'gpu_num') or old_flavor.get('gpu_mode') != new_flavor.get(
                'gpu_mode'):
            try:
    ...
    def modify_instance_vnic(self, fc_vm, old_flavor, new_flavor):
        if old_flavor.get('instance_bandwidth') != new_flavor.get(
                'instance_bandwidth') or old_flavor.get(
                'instance_max_vnic') != new_flavor.get(
                'instance_max_vnic'):
    ...
    def get_vnc_console(self, instance, get_opt):
        ...
        if not host_port or not host_ip:
            raise exception.\
                ConsoleNotFoundForInstance(instance_uuid=instance['uuid'])
    ...
    def attach_interface(self, instance, vif, extra_specs):
        ...
        vsp_body = {'name': vif['id'], 'portId': vif['id'],
            'portGroupUrn': pg_urn, 'mac': vif['address'],
            'virtIo': 1 if str(fc_vm.osOptions.get(
                'osVersion')) in constant.VIRTUAL_IO_OS_LIST else 0}
